...
function applyFrame(
  ...
  const marketsIn = clamp01((progress - PHRASE_END + 0.02) / 0.1);
  if (marketsHeadEl) {
    const showHead = progress >= PHRASE_END - 0.04;
    marketsHeadEl.classList.toggle('trader-scroll__markets-head--visible', showHead);
    marketsHeadEl.style.opacity = String(showHead ? marketsIn : 0);
  }
...
export function LandingTraderScroll({ id = 'mercados', onOpenMarket }: Props) {
  ...
  const update = useCallback(() => {
    ...
    applyFrame(progress, lines, barsRef.current, panels, marketsHeadRef.current);
  }, []);
  ...
  return (
    <div ref={rootRef} id={id} className="trader-scroll trader-scroll--markets" aria-label="Narrativa y mercados">
      <div className="trader-scroll__pin">
        ...
        <div className="trader-scroll__categories">
          {categories.map((cat, i) => (
            <article
              ...
              className="trader-scroll__category"
              aria-hidden="true"
            >
