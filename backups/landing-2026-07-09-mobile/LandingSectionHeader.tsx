...
export function LandingSectionHeader({
  ...
  return (
    <div className={`mx-auto max-w-3xl px-4 text-center ${className}`}>
      {eyebrow ? (
        goldenEyebrow ? (
          <GoldenHighlight as="p" variant="sm" className="text-sm font-semibold uppercase tracking-wide">
            {eyebrow}
          </GoldenHighlight>
        ) : sparkleEyebrow ? (
          <SparkleTitle
      ...
      {title ? (
        goldenTitle ? (
          <GoldenHighlight
            as="h2"
            variant="lg"
            className={`text-2xl font-bold sm:text-3xl ${eyebrow ? 'mt-2' : ''}`}
          >
            {title}
          </GoldenHighlight>
        ) : sparkleTitle ? (
